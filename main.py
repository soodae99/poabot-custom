...
def save_trade_record(order_info: MarketOrder, order_result: dict, bot):

    try:

        strategy = order_info.order_name

        exchange = order_info.exchange

        symbol = order_info.unified_symbol

        price = order_result.get('price') or order_result.get('average') or bot.get_price(symbol)

        amount = order_result.get('amount') or order_info.amount or order_info.amount_by_percent

        leverage = order_info.leverage or 1
        ...
        if order_info.is_entry:
            ...
            db.save_entry(strategy=strategy, exchange=exchange, symbol=symbol, side=side, amount=amount, price=price, leverage=leverage)

        elif order_info.is_close:

            result = db.close_position(strategy=strategy, exchange=exchange, symbol=symbol, exit_price=price, exit_amount=amount)

            if result:

                log_message(f"[{strategy}] 손익: {result['pnl']:.2f} USDT ({result['pnl_percent']:.2f}%)")

    except Exception as e:

        log_error_message(str(e), "거래 기록 저장 실패")
    ...
    try:
        ...
        if bot.order_info.is_crypto:
            ...
            if bot.order_info.is_entry:
                ...
                order_result = order_manager.execute_with_retry(bot.market_entry, bot.order_info, "entry")

            elif bot.order_info.is_close:

                order_manager = OrderManager(bot, max_retries=3, retry_delay=2)
                order_result = order_manager.execute_with_retry(bot.market_close, bot.order_info, "close")
            ...
            if order_info.is_futures and (order_info.is_entry or order_info.is_close):

                background_tasks.add_task(save_trade_record, order_info, order_result, bot)

        elif bot.order_info.is_stock:

            order_result = bot.create_order(bot.order_info.exchange, bot.order_info.base, order_info.type.lower(), order_info.side.lower(), order_info.amount)
...
async def hedge(hedge_data: HedgeData, background_tasks: BackgroundTasks):
    ...
    foreign_order_info = OrderRequest(exchange=exchange_name, base=base, quote=quote, side="entry/sell", type="market", amount=amount, leverage=leverage)
    ...
    if hedge == "ON":
        ...
        try:
            ...
            try:
                ...
                korea_order_info = OrderRequest(exchange="UPBIT", base=base, quote="KRW", side="buy", type="market", amount=binance_order_amount)
                ...
                binance_order_result = bot.market_close(OrderRequest(exchange=exchange_name, base=base, quote=quote, side="close/buy", amount=binance_amount))
        ...
        try:
            ...
            if binance_amount > 0 and upbit_amount > 0:
                ...
                order_info = OrderRequest(exchange="BINANCE", base=base, quote=quote, side="close/buy", amount=binance_amount)
